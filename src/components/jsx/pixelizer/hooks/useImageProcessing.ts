...
const useImageProcessing = () => {
	...
	const smoothMatrix = (matrix: RGBMatrix, factor: number): RGBMatrix => {
		const smoothed: RGBMatrix = [];
	...
	const spreadMatrix = (matrix: RGBMatrix, factor = 1): RGBMatrix => {
		const height = matrix.length;
		const width = matrix[0].length;

		// Create a blank matrix (filled with black or transparent pixels)
		const output: RGBMatrix = Array.from({ length: height }, () =>
	...
	const processImage = async (
		...
		processingMode: ProcessingMode = 'smooth'
