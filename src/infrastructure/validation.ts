...
export type ValidationFn = (v: unknown) => ValidationResult;

export type IsEmail = (v: unknown) => v is string;
export type IsUrl = (v: unknown) => v is string;
export type IsPositiveNumber = (v: unknown) => v is number;
...
export const validateFileType = (
	...
	if (!allowedTypes.includes(file.type)) {
		setError(`Please select a valid file (${allowedTypes.join(', ')})`);
		return false;
	}
	return true;
};
